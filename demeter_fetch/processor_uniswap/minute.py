...
def get_minute_df(config: Config, day: datetime.date, data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    input = data[UniNodesNames.pool]
    input_copy = input.copy()
    input_copy["block_timestamp"] = pd.to_datetime(input_copy["block_timestamp"])
    input_copy = input_copy.set_index(keys=["block_timestamp"])
    input_copy["tx_type"] = input_copy.apply(lambda x: uniswap_utils.get_tx_type(x.topics), axis=1)
    input_copy = input_copy[input_copy["tx_type"] == OnchainTxType.SWAP]
    ...
    ] = input_copy.apply(lambda r: uniswap_utils.handle_event(r.tx_type, r.topics, r.data), axis=1, result_type="expand")
